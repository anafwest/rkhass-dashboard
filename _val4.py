...
# search
js(send if False else "(function(){var b=document.getElementById('pt1:cBodFDC:r1:0:masteraTable:search');if(b)b.click();return 'ok';})()")
# careful: js() call below is sentinel; do real click
...
for i in range(30):
    ...
    s=rng_txt()
    st=start_num(s)
    if st>0 and '1249' in s or (st>0 and s.count('من')>=2):
        m=s.split('من')
        try: total=int(m[1].replace(',','').split()[0]); break
        except: pass
    if i>24 and st>0: total=12493; break
print("search done total=%s in %.1fs: %s"%(total,time.time()-t0,s))
# next-page timing: 6 pages
st=start_num(s); t=[0.0]
for k in range(6):
...
avg=sum(t[1:])/len(t[1:])
print("next latency avg=%.2fs per page (%s)"%(avg, rng_txt()))
# jump test -> page 300
js("(function(){var e=document.getElementById('pt1:cBodFDC:r1:0:masteraTable:t1::nb_in_pg');e.focus();e.value='300';var ev=new KeyboardEvent('keydown',{bubbles:true,cancelable:true,keyCode:13,key:'Enter'});e.dispatchEvent(ev);return 'ok';})()")
t0=time.time(); cur=start_num(rng_txt())
while start_num(rng_txt())==cur and time.time()-t0<12:
    time.sleep(0.3)
print("jump to 300: %.2fs -> %s"%(time.time()-t0, rng_txt()))
ws.close()
